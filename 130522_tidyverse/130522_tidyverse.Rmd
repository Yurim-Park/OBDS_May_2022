...
```{r}
iris_raw <- read_csv(file = "/project/obds/shared/resources/2_r/r_tidyverse/iris.csv")
```
...
```{r}
write_tsv(iris_raw, file="iris.tsv")
```
...
```{r}
iris_raw %>% group_by(species) %>% summarise(mean(sepal_length), median(sepal_length), min(sepal_length), max(sepal_length))
...
```{r}
iris_raw %>% group_by(species) %>% summarise(dplyr::across(starts_with("sepal"), max))
```


```{r}
iris_raw %>% group_by(species) %>% summarise(dplyr::across(where(is.numeric), mean))
...
iris_raw %>% group_by(species) %>% summarise(dplyr::across(where(is.numeric), mean)) %>% filter(sepal_length<6)
...
```{r}
# Copy the code chunk above and extend with more pipes
iris_raw %>% group_by(species) %>% summarise(dplyr::across(where(is.numeric), mean)) %>% filter(sepal_length<6) %>% arrange(desc(sepal_length))
...
```{r}
# Copy the code chunk above and extend with more pipes
iris_raw %>% group_by(species) %>% summarise(dplyr::across(where(is.numeric), mean)) %>% filter(sepal_length<6) %>% arrange(desc(sepal_length)) %>% pull(sepal_length, name = species)
...
```{r}
# Copy the code chunk above and extend with more pipes
iris_raw %>% mutate(sepal_area = sepal_length * sepal_width) %>% select(species, sepal_area) %>% slice_max(sepal_area, n=5)
...
```{r}
ggplot(iris_for_ggplot, aes(x=sepal_area, fill=species)) +geom_histogram()+facet_wrap(~species, ncol=2, nrow=2)
```
...
```{r}
iris_long <- iris_raw %>% pivot_longer(cols = where(is.numeric), names_to = "measurement", values_to = "numeric value")
...
iris_long2 <- iris_raw %>% mutate(ID = as.character(seq(1, nrow(iris)))) %>% pivot_longer(cols = where(is.numeric), names_to = "measurement", values_to = "numeric value")
...
# Copy the code chunk above and refine to address the issue
iris_long2 %>%
  pivot_wider(names_from = measurement, values_from = `numeric value`) %>%
  select(-ID)
...
```{r}
exprs_assay_long <- exprs_assay %>% pivot_longer(cols = starts_with("TCGA"), names_to = "unique_patient_ID", values_to = "expression_level")
...
```{r}
excel_sheets("/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx")
sample_info <- read_excel(path = "/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx", sheet = "sample_info")
sample_info
```
...
```{r}
sample_info <- sample_info %>% mutate(grade = as.factor(grade), T=as.factor(T), N=as.factor(N), M=as.factor(M))
...
```{r}
exprs_sampleinfo <- left_join(exprs_assay_long, sample_info)
...
```{r}
exprs_sampleinfo %>% group_by(gene, sex) %>% summarise(exp_mean = mean(expression_level))

exprs_sampleinfo %>% group_by(gene, sex) %>% summarise(exp_mean = mean(expression_level)) %>% group_by(gene) %>% summarise(max(exp_mean)-min(exp_mean))
...
```{r}
table_for_ggplot <- exprs_sampleinfo %>% select(expression_level, sex, T)
  
table_for_ggplot
```

```{r}
ggplot(table_for_ggplot, aes(x=expression_level, fill = T)) +geom_histogram()+facet_wrap(~sex)
...
```{r}
ggplot(exprs_sampleinfo, aes(x=unique_patient_ID, y=gene, fill = expression_level)) +
