...
```{r}
ggplot(pca_iris_dataframe,aes(x=PC1, y=PC2, color=PC1)) +geom_point()
...
head(iris)
head(pca_iris_dataframe)
pca_iris_dataframe <- cbind(pca_iris_dataframe, species = iris[,5])


```

```{r}
pca_iris_species <- ggplot(pca_iris_dataframe,aes(x=PC1, y=PC2, color=species)) +geom_point()
...
```{r}
pca_iris_dataframe <- cbind(pca_iris_dataframe, petal_length=iris$Petal.Length)
head(pca_iris_dataframe)
ggplot(pca_iris_dataframe,aes(x=PC1, y=PC2, color=petal_length))+geom_point()
...
```{r}
a<-pca_iris$sdev^2
b<-sum(pca_iris$sdev^2)
explained_variance_ratio <- a/b
...
head(umap_iris_dataframe)
umap_iris_species <- ggplot(umap_iris_dataframe,aes(x=UMAP1, y=UMAP2, color = species) ) +geom_point()
...
labels_colors(iris_hclust_dend) <-as.numeric(iris$Species)+3
...
cowplot::plot_grid(plot_average,plot_complete, plot_single, plot_ward, ncol=2, nrow=2
...
```{r}
dbscan_iris <- dbscan(iris_features, eps=1)
dbscan_iris
```
...
```{r}
hdbscan_iris <- hdbscan(iris_features, minPts = 4)
hdbscan_iris
```
...
```{r}
set.seed(1)
kmeans_iris <- kmeans(iris_features, center=3)
kmeans_iris
```
...
```{r}
ggplot(iris_labelled,aes(x=Sepal.Length, y=Sepal.Width, color=Kmeans)) +geom_point()+geom_point(aes(x=Sepal.Length, y=Sepal.Width, color = Kmeans), data=iris_means_centers, shape = "x", size=10) 
...
```{r}
table(
  kmeans_iris$cluster, iris$Species
)
```
...
```{r}
ggplot(kmean_totwithinss_dataframe,aes(K, totss)    ) +geom_point()
