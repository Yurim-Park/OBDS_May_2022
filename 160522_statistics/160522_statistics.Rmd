...
ggplot(data=x_dataframe, mapping=aes(x= x_column))+geom_histogram()+geom_vline(xintercept = mean(x), color = "red")+geom_vline(xintercept=median(x), color = "red", linetype = "dashed")+geom_vline(xintercept=mean(x)+sd(x)*c(-1,1), color="blue", linetype="dashed")
```
...
```{r}
norm_v<- pnorm(q=seq(-5,5, by=0.1), mean=0, sd=1)
gg_frame <- data.frame(value=norm_v, quantile=seq(-5,5, by=0.1))
...
gene_by_infection <- data.frame(expression = gene_values, infection = cell_infection)
out <- t.test(expression~infection, data=gene_by_infection)
str(out)
out$p.value
```
...
```{r}
df_t_test <- data.frame("p_values"=t_test_pvalues)

ggplot(df_t_test, aes(x=p_values))+geom_histogram()
```
...
summary(result_bh <0.05)
summary(t_test_pvalues<0.05)
```
...
fisher_test_pathway(index = 1,
  pathways = go_list,
  query = names(logcounts_matrix)[result_bh < 0.05],
  universe = rownames(logcounts_matrix))
